...
class TSaveCmd final : public ICmd {
public:
    TSaveCmd(Porto::Connection *api) : ICmd(api, "save", 1,
            "[-P] [-f <file>] <name>", "save container configuration",
            std::string() +
            "    -P         save into " + PORTO_CONTAINERS + "\n"
            "    -f <file>  save into file\n") { }

    int Execute(TCommandEnviroment *env) final override {
        bool place = false;
        std::string file;
        const auto &args = env->GetOpts({
                {'P', false, [&](const char *) { place = true; }},
                {'f', true,  [&](const char *arg) { file = arg; }},
        });
        ...
        ret = Api->Plist(plist);
        if (ret) {
            PrintError("Cannot list properties");
            return ret;
        }
        ...
        std::map<std::string, std::map<std::string, Porto::GetResponse>> result;
        ret = Api->Get({name}, properties, result, Porto::GetFlags::Real);
        if (ret) {
            PrintError("Cannot get container " + name);
            return ret;
        }
        ...
        for (auto &it: result[name]) {
            ...
            text += fmt::format("{} = {}\n", key, StringReplaceAll(val.Value, "\n", "\\n"));
        }

        if (place) {
            TPath dir(PORTO_CONTAINERS);
            if (!dir.Exists()) {
                error = dir.Mkdir(0755);
                if (error) {
                    PrintError(error, "Cannot create directory");
                    return -1;
                }
            }

            TPath path = dir / StringReplaceAll(name, "/", "%") + ".CT";
            error = path.WriteAtomic(text);
            if (error) {
                PrintError(error, "Cannot save container " + name);
                return -1;
            }
        }

        if (file.size()) {
            error = TPath(file).WriteAtomic(text);
            if (error) {
                PrintError(error, "Cannot save container " + name);
                return -1;
            }
...
class TLoadCmd final : public ICmd {
public:
    TLoadCmd(Porto::Connection *api) : ICmd(api, "load", 0,
            "[-P] [-f <file>] [-S] [name]", "load container configuration",
            std::string() +
            "    -P         load from " + PORTO_CONTAINERS + "\n"
            "    -f <file>  load from file\n"
            "    -S         start container\n") { }

    int Load(std::string &name, const std::string &file, bool start) {
        std::string text;
        TError error;
        int ret;

        if (file.empty()) {
            TFile f(STDIN_FILENO);
            error = f.ReadAll(text, 1 << 20);
            f.SetFd = -1;
        } else
            error = TPath(file).ReadAll(text);
        if (error) {
            PrintError(error, "Cannot read file " + file);
            return -1;
        }
        ...
        for (auto &line: lines) {
            ...
            auto key = StringTrim(line.substr(0, sep));
            auto val = StringReplaceAll(StringTrim(line.substr(sep + 1)), "\\n", "\n");
            if (key == "weak" || key == "root_pid" ||
                    key == "cpu_set_affinity")
                continue;
            if (key == "name") {
                if (name.empty())
                    name = val;
        ...
        if (name.empty()) {
            PrintError("Container name not defined");
            return -1;
        }

        Print("Load container " + name + " ...");

        if (Api->CreateWeakContainer(name)) {
            PrintError("Cannot create container");
            return -1;
        }
        ...
        for (auto &it: properties) {
            ...
            if (Api->SetProperty(name, key, val)) {
                PrintError("Cannot set property " + key);
        ...
        if (Api->SetProperty(name, "weak", "false")) {
            PrintError("Cannot clear weak flag");
            (void)Api->Destroy(name);
            return -1;
        }
    ...
    int Execute(TCommandEnviroment *env) final override {
        ...
        const auto &args = env->GetOpts({
                {'P', false, [&](const char *) { place = true; }},
                {'f', true,  [&](const char *arg) { file = arg; }},
                {'S', false, [&](const char *) { start = true; }},
        });
        TError error;
        int ret;

        if (!place) {
            auto name = args.size() ? args[0] : "";
            return Load(name, file, start);
        }

        if (file.size()) {
            PrintError("both options -P and -f are set");
            return -1;
        }

        TTuple files;
        bool found = false;
        error = TPath(PORTO_CONTAINERS).ReadDirectory(files);
        if (error) {
            PrintError(error, "Cannot list saved containers");
            return -1;
        }

        TTuple names;
        for (auto &file: files) {
            if (!StringEndsWith(file, ".CT"))
                continue;
            auto n = StringReplaceAll(file.substr(0, file.size() - 3), "%", "/");
            if (args.empty()) {
                names.push_back(n);
            } else {
                for (auto &pat: args) {
                    if (StringMatch(n, pat)) {
                        names.push_back(n);
                        break;
                    }
                }
        ...
        if (names.empty() && args.size()) {
            PrintError("Containers not found");
            return -1;
        }

        bool fail = false;
        for (auto &name: names) {
            file = std::string(PORTO_CONTAINERS) + "/" + StringReplaceAll(name, "/", "%") + ".CT";
            if (Load(name, file, start))
                fail = true;
        }
        if (fail)
            return -1;
...
int main(int argc, char *argv[]) {
    ...
    handler.RegisterCommand<TSaveCmd>();
    handler.RegisterCommand<TLoadCmd>();
