...
TError TClient::ReadRequest(Porto::TPortoRequest &request) {
    if (Fd < 0)
...
TError TClient::QueueResponse(Porto::TPortoResponse &response) {
...
TError TClient::QueueReport(const TContainerReport &report, bool async) {
    ...
    if (Verbose)
        L_RSP("{}Wait name={} state={} {}={} to {}", async ? "Async" : "", report.Name,
                wait->state(), report.Label, report.Value, Id);

    return QueueResponse(rsp);
}

TError TClient::MakeReport(const std::string &name, EContainerState state, bool async,
